...
We're going to provide descriptions for each of these locations, as well as provide metadata associated with them. We'll do that later with something called *records* in LFE. But for now, just to get some practice with some of the basic concepts in Lisp, let's pretend we defined three variables, each containing data for our locations:
...
Now, assume we wanted to bring all of these together in yet another, mind-blowingly cool variable, ``map``:
